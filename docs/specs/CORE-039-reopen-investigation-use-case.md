...
Expose the investigation reopening capability through the application layer.
...
The domain already supports reopening an investigation.

Application services should expose this behaviour without duplicating domain logic.

## Acceptance Criteria

- A `ReopenInvestigation` use case exists.
- The use case retrieves the investigation from the repository.
- The investigation is reopened through the domain model.
