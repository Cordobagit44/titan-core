...
class Investigation(Entity[DomainEvent]):
    ...
    def restore(
        ...
        claim_ids = [claim.id for hypothesis in hypotheses for claim in hypothesis.claims]
        ...
        if any(assessment.thesis_id not in set(thesis_ids) for assessment in assessments):
            raise ValueError("assessment thesis not found")
        ...
        investigation._assessments.extend(assessments)
    ...
    def add_claim(
        ...
        if any(
            ...
            for owner in self._hypotheses
            if owner.id != hypothesis.id
            for existing in owner.claims
        ):
            raise ValueError(
                "claim already belongs to another hypothesis",
            )
